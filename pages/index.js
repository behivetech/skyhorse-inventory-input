...
`
const getJsonValue = (value, unit) => JSON.stringify({ value, unit })
...
const Index = ({ shopOrigin }) => {
    ...
    const handleChange = useCallback((newChecked) => setStabilized(newChecked), []);
    const { control, formState: { errors }, handleSubmit, reset: resetForm } = useForm({
        defaultValues: { pricePerCarat: 2.00 },
    ...
    function renderHtmlRows(formData, index) {
        ...
        const detailRows = Object.keys(formFieldParams).map((key) => {
            ...
            return value && getDescriptionValue && `<tr><td>${label}</td><td>${getDescriptionValue(value)}</td></tr>`
        });

        if (!length && !width && !height) {
            detailRows.push('<tr><td colspan="2">Measurements available upon request through our contact form</td></tr>')
        }
    ...
    function getBarcode() {
        ...
        return [
            getRandomNumber(1),
            Math.floor(Date.now() / 1000 / 60).toString().slice(-6),
            getRandomNumber(2)
        ].join('')
    }
    ...
    function getProductInputs(formData) {
        ...
        return {
            variables: {
                input: {
                    // collectionsToJoin: [],
                    title: `${type}${stabilized ? ` (Stabilized)` : ''} - ${mine} - ${carat}ct`,
                    vendor: 'Skyhorse Industries',
                    descriptionHtml: `<table>${renderHtmlRows(formData).join('')}</table>`,
    ...
    return (
        <Page narrowWidth divider>
            ...
            {addProductData && (
                <Banner status="success" title="Product Successfully Created" onDismiss={handleSuccessBannerDismiss}>
                    <p>
                        Product Page: <Link url={getLink('productPage')} external>
                            {addProductData.productCreate.product.title}
                        </Link><br />
                        <Link url={getLink('barcode')} external>Print Barcode</Link>
                    </p>
                </Banner>
            )}
                        ...
                        <FormLayout>
                            ...
                            />
                            {
                                Object.keys(formFieldParams).map((key, index) => {
                                    const { inputFieldParams: { fieldType, options, placeholder }, label, type } = formFieldParams[key];
